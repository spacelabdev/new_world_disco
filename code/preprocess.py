...
def preprocess_tess_data(tess_id=DEFAULT_TESS_ID):
    ...
    for i in range(tce_count):
        ...
        export_data_to_s3(np.array(lc_local.flux), out, f'{str(tess_id)}_0{int(info[1])}_local')
        export_data_to_s3(np.array(lc_global.flux), out, f'{str(tess_id)}_0{int(info[1])}_global')
